...
void DrawToChart(HWND WindowHandle, HDC DeviceContext, SCStudyInterfaceRef sc)
{
	...
	sc.Graphics.DrawRectangle(sc.StudyRegionLeftCoordinate + 250, sc.StudyRegionTopCoordinate + 250, sc.StudyRegionLeftCoordinate + 500, sc.StudyRegionTopCoordinate + 500);


	n_ACSIL::s_GraphicsBrush GraphicsBrush2;
	GraphicsBrush2.m_BrushType = n_ACSIL::s_GraphicsBrush::BRUSH_TYPE_SOLID;
	GraphicsBrush2.m_BrushColor.SetRGB(0, 255, 0);

	sc.Graphics.SetBrush(GraphicsBrush2);

	GraphicsBrush2.m_BrushColor.SetRGB(0, 255, 128);

	sc.Graphics.SetBrush(GraphicsBrush2);

	n_ACSIL::s_GraphicsPen GraphicsPen2ForRectangle;
	GraphicsPen2ForRectangle.m_PenColor.SetColorValue(COLOR_WHEAT);
	GraphicsPen2ForRectangle.m_PenStyle = n_ACSIL::s_GraphicsPen::e_PenStyle::PEN_STYLE_SOLID;
	GraphicsPen2ForRectangle.m_Width = 5;

	sc.Graphics.SetPen(GraphicsPen2ForRectangle);

	//Draw a rectangle at the top left of the chart
	sc.Graphics.DrawRectangle(sc.StudyRegionLeftCoordinate + 550, sc.StudyRegionTopCoordinate + 550, sc.StudyRegionLeftCoordinate + 700, sc.StudyRegionTopCoordinate + 700);
