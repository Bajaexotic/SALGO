...
SCSFExport scsf_TradeManagementByStudy(SCStudyInterfaceRef sc)
{
	...
	SCInputRef Input_AdjustOrderImmediatelyOnEnable = sc.Input[16];
	...
	if (sc.SetDefaults)	
	{
		...
		Input_Enabled.DisplayOrder = DisplayOrder++;
		...
		Input_AdjustmentTiming.DisplayOrder = DisplayOrder++;

		Input_AdjustmentFrequency.Name = "Adjustment Frequency (in Seconds)";
		Input_AdjustmentFrequency.SetInt(5);
		Input_AdjustmentFrequency.DisplayOrder = DisplayOrder++;

		Input_AdjustOrderImmediatelyOnEnable.Name = "Adjust Order Immediately One Time";
		Input_AdjustOrderImmediatelyOnEnable.SetYesNo(0);
		Input_AdjustOrderImmediatelyOnEnable.DisplayOrder = DisplayOrder++;

		Input_TrailingOffset.Name = "Trailing Offset (in Ticks)";
		Input_TrailingOffset.SetInt(2);
		Input_TrailingOffset.DisplayOrder = DisplayOrder++;
		...
		Input_ACSButtonNumber.DisplayOrder = DisplayOrder++;
		...
		Input_AllowCustomPropertiesForControlBarButton.DisplayOrder = DisplayOrder++;
		...
		Input_AttachedOrderToManage.DisplayOrder = DisplayOrder++;
		...
		Input_ChangeToOffAllOtherCustomStudyButtonsOnEnable.DisplayOrder = DisplayOrder++;
	...
	bool AdjustOrderImmediatelyOnEnableOrNewPosition = Input_AdjustOrderImmediatelyOnEnable.GetBoolean();
	...
	int& r_ImmediateMoveDone = sc.GetPersistentInt(3);
	...
	if (AdjustByFixedTimeInSeconds && ((AdjustOrderImmediatelyOnEnableOrNewPosition && r_ImmediateMoveDone) || !AdjustOrderImmediatelyOnEnableOrNewPosition))
	{
	...
	if (Result == 1)
	{
		r_ImmediateMoveDone = 1;

		if (Input_DetailedLogging.GetYesNo() && LastLogMessageIdentifier != 11)
		{
